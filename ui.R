...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(width = 6,
               ...
               and the sum of errors (FNR + FPR) is minimized. You manipulate Se and Sp with the sliders."),
                   # actionButton("quit", label="Quit"),
                   column(6,
                          sliderInput("Se",
                                      "Sensitivity (Se) or True Positive Rate: proportion of true patients with test scores above the intersection:",
                                      min = .5,
                                      max = 1,
                                      value = .85,
                                      #width='50%'
                   ...
                   column(6,
                          sliderInput("Sp",
                                      "Specificity (Sp) or True Negative Rate: proportion of true non-patients with test scores below the intersection:",
                                      min = .5,
                                      max = 1,
                                      value = .85,
                                      #width = '50%'
    ...
    mainPanel(width = 6,
              ...
              tabsetPanel(
                tabPanel("Introduction", 
                         ...
                         p("This Shiny App is intended to provide a hands-on demonstration of the",
                           ...
                           It shows how dichotomous test accuracy translates to trichotomous test accuracy.
             ...
             the use of this method for evaluating your medical test."),
                ...
                tabPanel("Hands-on", 
                         ...
                         p(
             "Set the uncertainty criterion to .55 and check the checkbox to
             combine the two sliders. With the sliders you manipulate the
             overlap between the two distributions and the strength of the
             simulated test. A lower overlap defines a stronger test. For
             convenience, the", span("AUC statistic", style="color:blue"), "is
             also presented, which is a commonly used estimate of the strength
             of the test (using all test scores)."
                           ),
                         tags$ul(
                           tags$li("1. Make the test stronger by setting Se ans Sp both to .9. This reduces the overlap between 
               ...
               scores that hardly differentiate the two groups."),
                      ...
                      comparison to dichotomous optimal classification. Optimal dichotomous classification uses the 
                       ...
                       The Uncertain Interval becomes smaller when the test is stronger."), 
                           tags$li("3. Move the sliders to values lower than .9. Observe that 
                      ...
                      uncertain interval is not very useful."),
                           tags$li("4. Move the sliders to .64. The values shown for ", 
                                   span('Sp.UI and Se.UI are the 
             values actually reached', style="color:blue"), 
                                   "for the test scores within the Uncertain Interval, 
             not the allowed uncertainty that has been set. Also note that the 
             simulated test shows a large overlap for the scores of patients and 
                       non-patients. This simulated test is weak and in fact useless.")
                         ),
           p("Please note that the values for Se.MCI and Sp.MCI are always
           larger than Se.opt and Sp.opt. The meaning of this is that the
           Sensitivity and Specificity improves for the patients with a test
           score outside the Uncertain Interval, that is, the patients who
           actually receive a positive or negative classification."), p("Also
           note that the uncertain interval is always around the point of
           intersection, where the two distributions have equal densities. The
           point of intersection is also the optimal dichotomous cut-point
           where the sum of Se + Sp is maximized and therefore the sum of
           dichotomous classification errors (1 - Se + 1 - Sp) would be
           minimized. "), p('In the examples above, both distributions keep a
           standard deviation of 1. Remove the check from the check box to
           disconnect the two sliders. Set Se to .74 and Sp to .94. The
           obtained distributions are now N(0, 1) and N(3.31, 2.72). Observe
           that the Uncertain Interval remains centered around the point of
           intersection.'
                            ),
                         tags$ul(
             ...
             scores become inconsistent in their meaning for classification."),
                           p(
             'Please note that the values shown for ', span('Sp.UI and Se.UI
             are the values actually reached', style="color:blue"), 'for the
             test scores within the Uncertain Interval, not the allowed
             uncertainty that has been set. If the deviation is considerable,
             the Uncertain Interval is invalid.', 'When the test score
             variance differ for both groups, the sum of Se.MCI and Sp.MCI is
             always greater than the sum of Se.opt and Sp.opt; for weak tests
             they can be equal. N.B. Sp.MCI and Se.MCI are calculated in the
             same way as Sp.VR and Se.VR for for the Virtual range of TG-ROC.'
             )
             )
                ), 
             ...
             Se slider sets the proportion of patients with test scores 
             above the point of intersection (blue dotted line in the left graph). The 
             Sp slider sets the percentage of patients which are known to not 
             ...
             sliders seperately, allowing the two distributions to have different variances.'),
             ...
             always to the left and right of the point of intersection.'),
                         p('Consequently, the trichotpomous Sensitivity and Specificity of the Uncertain Interval are 
               directly connected to the optimal dichotomous Sensitivity and Specificity:'), 
             ...
             This UI indicates a range of test scores that cannot sufficiently 
             ...
             with a mean of 2.07 and a standard deviation of 1 (N(2.07, 1))."),
                ...
                probability is assumed to .50. In other words, before testing  it is considered 
                equally likely that the patient may or may not have the 
                disease. It is assumed that the patient is drawn from a clinical
                population, such as the patients who have been refreed to a department
                of a hospital. The method is therefore not fit for estimation of the probability
             ...
             are not used for a positive or negative classification. In many cases these 
